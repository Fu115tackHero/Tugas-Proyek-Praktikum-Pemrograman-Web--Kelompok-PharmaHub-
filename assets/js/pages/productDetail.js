...
function loadProductData(productId) {
  ...
  // Update tab contents
  document.getElementById("precaution-content").innerHTML = product.precaution
    .map((item) => `<p>• ${item}</p>`)
    .join("");
  ...
  document.getElementById("indication-content").innerHTML = product.indication
    .map((item) => `<p>• ${item}</p>`)
    .join("");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (checkoutLink) {
    checkoutLink.addEventListener("click", (e) => {
      ...
      // Save to localStorage as cart
      localStorage.setItem("pharmahub-cart", JSON.stringify(tempCart));
