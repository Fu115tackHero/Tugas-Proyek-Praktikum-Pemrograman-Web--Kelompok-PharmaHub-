...
function renderCart() {
  ...
  cartItemsContainer.innerHTML = cart
    .map((item) => {
      ...
      const savedBadge = isDisabled
        ? '<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600 ml-2 opacity-50"><i class="fas fa-bookmark mr-1"></i>Saved for later</span>'
        : "";
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center mb-1">
                        <h3 class="font-semibold text-gray-800 ${
                          isDisabled ? "text-gray-500 opacity-50" : ""
                        }">${item.name}</h3>
                        ${savedBadge}
                    </div>
                    <p class="text-sm text-gray-600 mb-2 ${
                      isDisabled ? "opacity-50" : ""
                    }">Obat untuk kesehatan</p>
                    ...
                    <div class="flex items-center space-x-2 mb-2">
                        ...
                        <div class="flex items-center border border-gray-300 rounded ${
                          isDisabled ? "opacity-50 pointer-events-none" : ""
                            ...
                            <span class="w-12 text-center py-1 text-sm">${
                              item.quantity
                            }</span>
                            <button onclick="updateQuantity('${item.id}', 1)" 
                                    class="quantity-btn w-8 h-8 flex items-center justify-center hover:bg-gray-100"
...
function applyCoupon() {
  ...
  if (subtotal < coupon.minAmount) {
    showCouponMessage(
      `Minimal belanja ${formatCurrency(
        coupon.minAmount
      )} untuk menggunakan kupon ini`,
      "error"
    );
    return;
  }
  ...
  showCouponMessage(
    `Kupon berhasil diterapkan! Diskon ${coupon.discount}%`,
    "success"
  );
...
function saveForLater(id) {
  ...
  if (item) {
    ...
    const action = item.savedForLater
...
function checkout() {
  ...
  alert(
    "Fitur checkout akan segera hadir! Terima kasih telah berbelanja di PharmaHub."
  );
}
...
function loadProductDetails() {
  ...
  if (elements.price)
    elements.price.textContent = formatCurrency(product.price);
  ...
  if (user) {
    ...
    if (navProfileImage) {
      ...
      if (profileData.profileImage) {
        ...
        navProfileImage.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(
          userName
